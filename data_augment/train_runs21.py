"""
Runs21: YOLOv11s Backbone + Cross-Scale Attention (per-image softmax weighting) + SupCon
==========================================================================================
Architecture:
  - Extract P3 (256-d), P4 (256-d), P5 (512-d) via GAP
  - Project all to common 512-d space: P3→512, P4→512, P5→512
  - Compute per-image softmax attention scores α = softmax([a3,a4,a5])
  - Weighted sum: h = α3*P3' + α4*P4' + α5*P5'  → h ∈ R^512
  - SupCon with Projection(512→256→128), Classifier Linear(512→2)
"""
...
FEAT_DIM  = 512   # Cross-scale attention output is 512-d

logging.basicConfig(filename=f"{OUT_DIR}/training.log", filemode="w", level=logging.INFO,
                    format="%(asctime)s - %(message)s")
log = logging.getLogger()
log.info("Runs21: Cross-Scale Attention + SupCon")
...
class CrossScaleAttentionBackbone(nn.Module):
    """Project each scale to 512-d, then softmax-weight and sum → h ∈ R^512"""
    def __init__(self, yolo_path):
        ...
        # Project to common 512-d space
        self.proj3 = nn.Linear(256, 512)
        self.proj4 = nn.Linear(256, 512)
        self.proj5 = nn.Linear(512, 512)
        # Scalar attention scores (learned, per-scale): a3, a4, a5
        self.scale_scores = nn.Parameter(torch.zeros(3))  # → softmax
    ...
    def forward(self, x):
        ...
        p3 = self.proj3(h3)   # [B, 512]
        p4 = self.proj4(h4)   # [B, 512]
        p5 = self.proj5(h5)   # [B, 512]
        # Softmax attention over 3 scales
        alpha = F.softmax(self.scale_scores, dim=0)  # [3]
        h = alpha[0] * p3 + alpha[1] * p4 + alpha[2] * p5  # [B, 512]
        return h
...
def finetune(model, name, lr_bb, lr_cls):
    ...
    return model  # return so we can log attention weights

# ── SUPCON ─────────────────────────────────────────────────────────────────────
log.info("Loading cross-scale backbone...")
bb = CrossScaleAttentionBackbone(YOLO_WEIGHTS).to(DEVICE)
supcon = SupConModel(bb).to(DEVICE)

log.info("Pre-SupCon t-SNE..."); e,l=get_embeds(bb,te_loader)
plot_tsne(e,l,"t-SNE: Pre-SupCon (Runs21)",f"{OUT_DIR}/tsne_pre.png")
...
for ep in range(1,SUPCON_EP+1):
    ...
    sched.step()
    if ep%10==0 or ep==1:
        alpha=F.softmax(bb.scale_scores,dim=0)
        log.info(f"SupCon Ep[{ep}/{SUPCON_EP}] Loss={tot/len(sc_loader):.4f}  α=[{alpha[0]:.3f},{alpha[1]:.3f},{alpha[2]:.3f}]")

# Log final attention weights
alpha_final = F.softmax(bb.scale_scores.detach(), dim=0)
log.info(f"FINAL attention weights: α3={alpha_final[0]:.4f} α4={alpha_final[1]:.4f} α5={alpha_final[2]:.4f}")
...
plot_tsne(e,l,"t-SNE: Post-SupCon (Runs21)",f"{OUT_DIR}/tsne_post.png")

# ── FINE-TUNING ────────────────────────────────────────────────────────────────
bb0=CrossScaleAttentionBackbone(YOLO_WEIGHTS); finetune(Classifier(bb0),"baseline",1e-5,1e-3)

bb_fr=CrossScaleAttentionBackbone(YOLO_WEIGHTS)
...
bb_pt=CrossScaleAttentionBackbone(YOLO_WEIGHTS)
...
bb_pt.scale_scores.requires_grad_(True)
finetune(Classifier(bb_pt),"supcon_partial",1e-5,1e-3)

bb_fu=CrossScaleAttentionBackbone(YOLO_WEIGHTS)
bb_fu.load_state_dict(torch.load(f"{OUT_DIR}/backbone_supcon.pth"))
m_fu = finetune(Classifier(bb_fu),"supcon_full",1e-5,1e-3)

# Log final learned attention weights after finetuning
alpha_ft = F.softmax(bb_fu.scale_scores.detach(), dim=0)
log.info(f"Post-finetune (full) attention weights: α3={alpha_ft[0]:.4f} α4={alpha_ft[1]:.4f} α5={alpha_ft[2]:.4f}")

log.info("Runs21 COMPLETE.")
