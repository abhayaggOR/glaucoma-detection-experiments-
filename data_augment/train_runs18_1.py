"""
Runs18.1: YOLOv11s + L2-Normalized Multi-Scale (P3+P4+P5) + SupCon
====================================================================
Fix: L2-normalize P3, P4, P5 independently before concatenation to
eliminate feature magnitude imbalance seen in Runs18.
Native dims: P3=256, P4=256, P5=512 → concat h ∈ R^1024
...
FEAT_DIM  = 1024  # 256+256+512 after L2 normalize + concat

logging.basicConfig(filename=f"{OUT_DIR}/training.log", filemode="w", level=logging.INFO,
                    format="%(asctime)s - %(message)s")
log = logging.getLogger()
log.info("Runs18.1: Normalized Multi-Scale SupCon @ 1024x1024")
...
class NormMultiScaleBackbone(nn.Module):
    """Extract P3/P4/P5 via GAP, L2-normalize each, then concat → 1024-d"""
    ...
    def forward(self, x):
        ...
        # L2 normalize independently before concat (KEY FIX vs Runs18)
...
# SUPCON
log.info("Loading backbone..."); bb=NormMultiScaleBackbone(YOLO_WEIGHTS).to(DEVICE)
supcon=SupConModel(bb).to(DEVICE)
log.info("Pre-SupCon t-SNE..."); e,l=get_embeds(bb,te_loader)
plot_tsne(e,l,"t-SNE Pre-SupCon (Runs18.1)",f"{OUT_DIR}/tsne_pre.png")
...
plot_tsne(e,l,"t-SNE Post-SupCon (Runs18.1)",f"{OUT_DIR}/tsne_post.png")

# FINETUNE
bb0=NormMultiScaleBackbone(YOLO_WEIGHTS); finetune(Classifier(bb0),"baseline",1e-5,1e-3)

bb_fr=NormMultiScaleBackbone(YOLO_WEIGHTS)
...
bb_pt=NormMultiScaleBackbone(YOLO_WEIGHTS)
