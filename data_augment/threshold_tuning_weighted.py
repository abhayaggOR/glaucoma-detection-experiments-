"""
Threshold Tuning — Class-Weighted YOLO v11 Models
===================================================
Evaluates YOLO11s-cls and YOLO11l-cls (trained with class-weighted CE
on augmented data) across a range of confidence thresholds on the TEST set.

Thresholds: 0.10, 0.15, 0.20, ..., 0.55
Results saved under data_augment/runs4/
"""
...
RUNS3_DIR = os.path.join(BASE_DIR, "runs3")
RUNS4_DIR = os.path.join(BASE_DIR, "runs4")
os.makedirs(RUNS4_DIR, exist_ok=True)

# Trained model weights (from class-weighted training in runs3)
MODELS = {
    "yolo11s_weighted": os.path.join(RUNS3_DIR, "yolo11s_weighted", "weights", "best.pt"),
    "yolo11l_weighted": os.path.join(RUNS3_DIR, "yolo11l_weighted", "weights", "best.pt"),
}
...
for model_name, model_path in MODELS.items():
    ...
    csv_path = os.path.join(RUNS4_DIR, f"{model_name}_threshold_results.csv")
    ...
    for normed, cm_data, suffix in [
        ...
        fig.tight_layout()
        png_path = os.path.join(RUNS4_DIR, f"{model_name}_{suffix}.png")
...
report = """# Threshold Tuning Report — Class-Weighted Models
...
report_path = os.path.join(RUNS4_DIR, "Threshold_Tuning_Report.md")
