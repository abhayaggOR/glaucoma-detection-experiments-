"""
Runs20.1: YOLOv11s + Normalized Multi-Scale + Lightweight SE Block (residual) + SupCon
========================================================================================
Fix Runs20 overfitting with: smaller SE (r=16, squeeze=64), Dropout(0.3), residual connection.
h_att = SE(h); h_out = h + h_att (residual prevents over-suppression)
Dims: P3=256, P4=256, P5=512 → normalize → concat h ∈ R^1024
Image: 1024x1024, batch=32, epochs=125
"""
...
OUT_DIR      = f"{BASE_DIR}/runs20_1"
TRAIN_DIR, TEST_DIR = f"{BASE_DIR}/train", f"{BASE_DIR}/test"
os.makedirs(OUT_DIR, exist_ok=True)

DEVICE   = torch.device("cuda" if torch.cuda.is_available() else "cpu")
IMG_SIZE = 1024; BATCH = 32; SUPCON_EP = 125; FT_EP = 200; PATIENCE = 100; TEMP = 0.07
FEAT_DIM = 1024  # 256+256+512

logging.basicConfig(filename=f"{OUT_DIR}/training.log", filemode="w", level=logging.INFO,
                    format="%(asctime)s - %(message)s")
log = logging.getLogger()
log.info("Runs20.1: Normalized Multi-Scale + Lightweight SE (residual, dropout) @ 1024x1024")
...
class LightweightSE(nn.Module):
    """Lightweight SE: dim→112→dropout→dim with residual connection"""
    def __init__(self, dim, squeeze=112, dropout=0.3):
        super().__init__()
        self.se = nn.Sequential(
            nn.Linear(dim, squeeze), nn.ReLU(True),
            nn.Dropout(dropout),
            nn.Linear(squeeze, dim), nn.Sigmoid())
    def forward(self, x):
        return x + x * self.se(x)  # residual: h = h + h * attention

class NormLightSEBackbone(nn.Module):
    def __init__(self, yolo_path):
        ...
        self.se     = LightweightSE(FEAT_DIM, squeeze=112, dropout=0.3)
    ...
    def forward(self, x):
        ...
        h5 = F.normalize(h5, dim=1)
        h  = torch.cat([h3, h4, h5], dim=1)   # 1024-d
        return self.se(h)                        # residual SE output
...
# SUPCON
log.info("Loading backbone..."); bb=NormLightSEBackbone(YOLO_WEIGHTS).to(DEVICE)
supcon=SupConModel(bb).to(DEVICE)
log.info("Pre-SupCon t-SNE..."); e,l=get_embeds(bb,te_loader)
plot_tsne(e,l,"t-SNE Pre-SupCon (Runs20.1)",f"{OUT_DIR}/tsne_pre.png")
...
plot_tsne(e,l,"t-SNE Post-SupCon (Runs20.1)",f"{OUT_DIR}/tsne_post.png")

# FINETUNE
bb0=NormLightSEBackbone(YOLO_WEIGHTS); finetune(Classifier(bb0),"baseline",1e-5,1e-3)

bb_fr=NormLightSEBackbone(YOLO_WEIGHTS)
...
bb_pt=NormLightSEBackbone(YOLO_WEIGHTS)
