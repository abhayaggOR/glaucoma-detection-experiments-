"""
Runs19.2: YOLOv11s + Normalized Multi-Scale + Learnable Scale Weights (biased init) + SupCon
==============================================================================================
Build on Runs18.1 by adding learnable scale weights initialized with domain prior:
  w3=1.2 (P3 fine-grained → higher priority)
  w4=1.0
  w5=0.8 (P5 global → slightly lower)
Native dims: P3=256, P4=256, P5=512 → normalized → weighted → concat h ∈ R^1024
Image size: 1024x1024, batch=32, epochs=150
"""
...
FEAT_DIM = 1024  # 256+256+512 after norm+concat

logging.basicConfig(filename=f"{OUT_DIR}/training.log", filemode="w", level=logging.INFO,
                    format="%(asctime)s - %(message)s")
log = logging.getLogger()
log.info("Runs19.2: Normalized Multi-Scale + Learnable Weights (biased init) @ 1024x1024")
...
class NormWeightedBackbone(nn.Module):
    """L2-normalize each scale, apply learnable weights (biased init), concat → 1024-d"""
    def __init__(self, yolo_path):
        ...
        # Biased init: P3 fine-grained features given higher priority
        self.w3 = nn.Parameter(torch.tensor(1.2))
        self.w4 = nn.Parameter(torch.tensor(1.0))
        self.w5 = nn.Parameter(torch.tensor(0.8))
    ...
    def forward(self, x):
        ...
        # L2 normalize then apply learnable weights
        h3 = self.w3 * F.normalize(h3, dim=1)
        h4 = self.w4 * F.normalize(h4, dim=1)
        h5 = self.w5 * F.normalize(h5, dim=1)
        return torch.cat([h3, h4, h5], dim=1)  # 1024-d
...
def finetune(model, name, lr_bb, lr_cls, bb_ref=None):
...
# SUPCON
log.info("Loading backbone..."); bb=NormWeightedBackbone(YOLO_WEIGHTS).to(DEVICE)
supcon=SupConModel(bb).to(DEVICE)
log.info("Pre-SupCon t-SNE..."); e,l=get_embeds(bb,te_loader)
plot_tsne(e,l,"t-SNE Pre-SupCon (Runs19.2)",f"{OUT_DIR}/tsne_pre.png")
...
for ep in range(1,SUPCON_EP+1):
    ...
    sched.step()
    if ep%10==0 or ep==1:
        log.info(f"SupCon Ep[{ep}/{SUPCON_EP}] Loss={tot/len(sc_loader):.4f}  w3={bb.w3.item():.4f} w4={bb.w4.item():.4f} w5={bb.w5.item():.4f}")
log.info(f"Final weights: w3={bb.w3.item():.4f} w4={bb.w4.item():.4f} w5={bb.w5.item():.4f}")
...
plot_tsne(e,l,"t-SNE Post-SupCon (Runs19.2)",f"{OUT_DIR}/tsne_post.png")

# FINETUNE
bb0=NormWeightedBackbone(YOLO_WEIGHTS); finetune(Classifier(bb0),"baseline",1e-5,1e-3)

bb_fr=NormWeightedBackbone(YOLO_WEIGHTS)
...
bb_pt=NormWeightedBackbone(YOLO_WEIGHTS)
